...
namespace scanner {
    ...
    std::vector<sig_byte> ida_to_bytes(std::string_view signature)
    // Turns IDA Signature to an easier form to search with
    {
    ...
    void *scan(std::string_view signature, std::string_view name, HMODULE mod) {
        ...
        MessageBoxA(0, std::format("Couldn't find signature {}", name).c_str(), "PizdaKot", 0);
        return nullptr;
    }
}
